class RenderHandlers{
    ...
    createOrderTile(order, index){
        const template = `
                ...
                <td>
                    <div class="user-credential">
                        <div class="credential-container">${order.userCredential}</div>
                    </div>
                </td>
                <td class="">
        ...
        order.products.forEach( (x) => {
            ...
            tr.querySelector('.product-container').insertAdjacentHTML('beforeend', tile)
        })

        tr.querySelector('.details-btn').addEventListener('click', this.displayOrderDetailsModal(order))
        return tr;
    }
    ...
    displayOrderDetailsModal(order){
        return async()=>{
            // fetch details
            const {data} = await fetchData("http://localhost:3000/admin/orders/complete_order_details/" + order._id);
            if(data){
                ...
                updateExpectedDeliveryDate.addEventListener('click', this.updateExceptedDeliveryDate(data))
                // add event listener
            }
        }
    }
    ...
    updateExceptedDeliveryDate(order){
        return ()=>{
            ...
            if(this.isSameDate(dateObj, selectedDateObj)){
               ...
               fetch("http://localhost:3000/admin/orders/update_estimated_delivery_date?id="+order._id+"&newExtimatedDate="+selectedDateObj.toISOString(), {
                    method: "PATCH", 
                    headers: {
               ...
               .then( data => {
                    if(data.isSuccess){
                        ...
                        this.updateCurrentOrderSet(order._id, selectedDateObj.toISOString());
                        // update 
                    }
                    else{
    ...
    async populateModalOrder(data){
        ...
        orderModal.querySelector('.user-address .user-name').innerHTML = data.userAddressId.fullName
        orderModal.querySelector('.user-address .actual-address').innerHTML = data.userAddressId.addressLine1 + ", " + data.userAddressId.addressLine2 + ", " + data.userAddressId.state
        orderModal.querySelector('.user-address .mobile').innerHTML = data.userAddressId.mobileNumber
        
        orderModal.querySelector('.products-container').innerHTML = ""
        orderModal.querySelector('.product-invoiceContainer').innerHTML = ""
        data.products.forEach( x => {
            orderModal.querySelector('.products-container').insertAdjacentHTML('beforeend', this.createModalProductTile(x))
        ...
        if(this.orderStates.length == 0){
            const {data} = await fetchData("http://localhost:3000/admin/orders/get_orderstages");
    ...
    createProductTile(product){
        const template = `<div class="productDiv d-flex gap-2 mb-2">
                        <div class="image-container">
                            <img width="50px" src="${product.product.images ? product.product.images[0] : ""}" alt="">
                        </div>
                        <div class="name-container responsive-text minimize">${product.productName}</div>
                    </div>`
    ...
    configureButton(){
        ...
        orderSearchButton.addEventListener('click', this.searchOrderHandler.bind(this))
        document.querySelector('.cancel-search-btn.order').addEventListener('click', this.removeSearch.bind(this))
    }

    searchOrderHandler(){
        const key = orderSearchInput.value;
        const url = "http://localhost:3000/admin/orders/search_order?searchKey=" + key
        ...
        .then( data => {
            if(data.isSuccess){
                ...
                document.querySelector('.cancel-search-btn.order').classList.remove('d-none')
            }
            else{
                showModel(data.errorMessage)
            }
    ...
    datePicker(defaultDate) {
        ...
        datePickerInput.value = defaultDateObj.getDate() + "-" + (defaultDateObj.getMonth()+1 < 10 ? `0${defaultDateObj.getMonth()+1}` : defaultDateObj.getMonth()+1) + "-" + defaultDateObj.getFullYear()
        ...
        $(datePickerInput).datepicker({
            ...
            defaultViewDate: { year: defaultDateObj.getFullYear(), month: defaultDateObj.getMonth(), day: defaultDateObj.getDate() }
        });
    }
}
