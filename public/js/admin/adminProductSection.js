class ProductHandlers {
  ...
  configureButton() {
    ...
    document
      .querySelector(".cancel-search-btn.product")
  ...
  searchProductHandler() {
    ...
    console.log(key);
    const url =  "/admin/search_product?searchKey=" + key;
  ...
  addProductHandler = async () => {
    ...
    let requestHelper = () => {
      ...
      const url =  "/admin/create_product";
      fetch(url, {
        method: "POST",
        // headers: {
  ...
  checkProductRequiredFields(value) {
    if (
      !(
        ...
        value.modelName &&
        value.actualPrice &&
        (value.discount || value.currentPrice)
      )
    ) {
      return {
        status: false,
        error: "Error: Please enter all required fields",
      };
    }
    if (value.currentPrice <= 0 || value.actualPrice <= 0) {
      return {
        status: false,
        error:
          "Error: Please check the price fields and retry. Price values cannot be negative",
      };
    }
    if (
      typeof value.currentPrice == "String" ||
      typeof value.actualPrice == "String"
    )
      if (value.currentPrice > value.actualPrice) {
        console.log(value.currentPrice, vlaue.actualPrice);
        return {
          status: false,
          error: "Error: Current price cannot be greater than actual price!",
        };
      }
    if (value.discount < 0 || value.discount > 100) {
      return {
        status: false,
        error: "Error: Discount have to be in the range 0 to 100",
      };
    }
    return { status: true };
  }
  ...
  updateProductHandler(currentValue) {
    ...
    return () => {
      ...
      console.log(value);
      if (status) {
        const url =  "/admin/update_product";
        const body = {
          id: currentValue._id,
          updateObject: value,
        };
  ...
  deleteImageEvent(id, src, imageCont) {
    return () => {
      const url =  "/admin/delete_image/" + id;
      fetch(url, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  deleteProductEvent(id) {
    // request to delete
    return () => {
      const url =  "/admin/delete_product/" + id;
      fetch(url, {
        method: "DELETE",
      })
        .then((response) => response.json())
  ...
  async updateModalData(value, heading, purpose) {
    ...
    if (purpose == "view") {
      ...
      deleteProduct.addEventListener(
        "click",
        this.deleteProductEvent(value._id)
      );
      ...
      addImageBtn.addEventListener(
        "click",
        this.addSingleImageEvent(value._id)
      );
  ...
  injectCategory(categoryList, id) {
    ...
    product_category.append(
      this.createCategoryTile({ category: "Choose category", value: "" })
    );
