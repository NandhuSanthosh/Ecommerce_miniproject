class CategoryHandler {
  ...
  addButtonConfiguration() {
    ...
    document
      .querySelector(".cancel-search-btn.category")
  ...
  searchProductHandler() {
    ...
    console.log(key);
    const url = process.env.URL + "/admin/search_category?searchKey=" + key;
  ...
  async addCategoryEvent() {
    ...
    const url = process.env.URL + "/admin/create_category";
    try {
      if (cate == "" || desc == "") {
  ...
  createCategoryTile(value, index) {
    ...
    tdTag
      .querySelector("p")
      .addEventListener("click", this.setTextExtendFeature);
    ...
    button.addEventListener(
      "click",
      this.renderIndividualCategoryDetails(value)
    );
  ...
  updateModalData(value) {
    ...
    deleteCategoryBtn.addEventListener(
      "click",
      this.deleteCategoryEvent(value._id)
    );
    updateCategoryBtn.classList.add("disabled");
    updateCategoryBtn.addEventListener(
      "click",
      this.updateCategoryEvent(value)
    );
  }
  ...
  validateUpdateCategoryValues(value) {
    ...
    if (
      (category.value == value.category &&
        description.value == value.description &&
        offer_details.value == value.offer) ||
      category.value == "" ||
      description.value == "" ||
      offer_details.value == ""
    ) {
  ...
  updateCategoryEvent(value) {
    return () => {
      ...
      const url = process.env.URL + "/admin/update_category/" + value._id;
  ...
  deleteCategoryEvent(id) {
    return () => {
      ...
      if (status) {
        const url = process.env.URL + "/admin/delete_category";
        fetch(url, {
          method: "delete",
          headers: {
            "Content-Type": "application/json",
