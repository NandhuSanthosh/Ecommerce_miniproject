...
class CoupenHandlers{
    ...
    configureButton(){
        add_coupon_btn.addEventListener('click', this.add_product_button_event.bind(this));
        searchCouponInput.addEventListener('keydown', (e)=>{
            if(e.key == "Enter"){
                this.searchCouponHandler();
            }
        })
        searchCouponBtn.addEventListener('click', this.searchCouponHandler.bind(this))
        document.querySelector('.cancel-search-btn.coupon').addEventListener('click', this.removeSearch.bind(this))
    ...
    searchCouponHandler(){
        ...
        .then( data => {
            ...
            if(data.isSuccess){
                ...
                document.querySelector('.cancel-search-btn.coupon').classList.remove('d-none')
            }
            else{
    ...
    createCouponTile(value, index){
        ...
        tableRow.innerHTML = `
                <th scope="row">${index+1}</th>
                    <td><p class="responsive-text minimize">${value.code}</p></td>
                    <td>${value.discount.discountType == "percentage-discount" ? value.discount.percentage + " %" : "₹" + value.discount.amount}</td>
    ...
    add_product_button_event(){
        ...
        create_coupon_btn.addEventListener('click', this.createCouponHandler.bind(this))
        displayCouponModal();
    }
    ...
    async createCouponHandler(){
        ...
        if(code.length <= 5 || !(/^\S+$/.test(code))) errorArray.push("Invalid coupon code");

        let discountType = coupon_discountType.value;
        if(discountType != "percentage-discount" && discountType != "amount-discount") errorArray.push("Invalid discount type")
        
        
        let discount = coupon_discount.value;
        let percentage, amount
        if(!discount || discount < 0 || (discount > 100 && discountType == "percentage-discount"))
        ...
        if(minSpend < 0) errorArray.push("Minimun Spend value should be positive.")

        let usageLimit = coupon_useageLimit.value;
        if(usageLimit < 0) errorArray.push("Usage limit should be positive or zero.")
    ...
    async updateFieldValue(value){
        ...
        if(categories.length){
            categories.map( x => {
                ...
                div.querySelector('button').addEventListener('click', removeCategory(x._id, div).bind(this))
                category_list_container.append(div)
            })
        }
        ...
        if(usedUsers.length){
            ...
            usedUsers.map( x => {
                ...
                div.innerHTML = `
                <span>${x.credentials.email || x.credentials.mobile.number}</span>`
    ...
    findALlUpdatedFields(value){
        ...
        if(coupon_discount.value != (coupon_discountType.value == "percentage-discount" ? value.discount.percentage : value.discount.amount)){
            updatedFields.updates.percentage = coupon_discount.value
            updatedFields.updates.amount = coupon_discount.value
        }
        ...
        if(!this.arrayEquals(this.currentCouponCategory, value.categories.map( x => x._id))){
        ...
        if(coupon_expiry_date.value != this.createFormatedDate(value.expiry)){
            updatedFields.updates.expiry = this.createDateObj(coupon_expiry_date.value);
        }
    ...
    createFormatedDate(defaultDate){
        ...
        return defaultDateObj.getDate() + "-" + (defaultDateObj.getMonth()+1 < 10 ? `0${defaultDateObj.getMonth()+1}` : defaultDateObj.getMonth()+1) + "-" + defaultDateObj.getFullYear()
    }
