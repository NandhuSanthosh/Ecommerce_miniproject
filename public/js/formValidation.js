  ...
  if (page == "login" || page == "signup") {
    ...
    validationIntegrateObj.forEach((field) => {
      field.field.addEventListener(
        "input",
        field.validityFunction(field.field)
      );
    });
  }
}
...
function getLoginUrl() {
  if (associate == "user") {
    return {
      reqUrl:  "/login",
      successUrl:  "/otp-auth?superSet=login",
    };
  }
  return {
    reqUrl:  "/admin/login",
    successUrl:  "/admin/otp-auth?superSet=login",
  };
}
...
signupBtn?.addEventListener("click", (e) => {
  ...
  if (status) {
    ...
    const referalCode = document
      .querySelector(".referal-code-input")
      .value.trim();
