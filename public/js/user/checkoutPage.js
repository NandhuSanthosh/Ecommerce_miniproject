...
function injectInsightDetails(order) {
  document.querySelector(".without-discounts").innerHTML =
    "₹" + order.totalPrice.toLocaleString();
  document.querySelector(".discount").innerHTML =
    "₹" + order.discount.toLocaleString();
  document.querySelector(".delivery-charge").innerHTML =
    order.delivery.deliveryCharge;
  if (order.delivery.isFreeDelivery) {
    document
      .querySelector(".free-delivery-container")
      .classList.remove("d-none");
    document.querySelector(
      ".free-delivery"
    ).innerHTML = `- ${order.delivery.deliveryCharge}`;
  }
  ...
  document.querySelector(".total .amount").innerHTML =
    "₹" + order.payable.toLocaleString();
  document.querySelector(".discount-figure").innerHTML =
  ...
  document
    .querySelector(".apply-coupen-btn")
    .addEventListener("click", applyCoupenCode(order._id));
  document
    .querySelector(".remove-coupen-btn")
    .addEventListener("click", removeCoupenCode(order._id));
  ...
  document
    .querySelector(".coupen-input-show-btn")
  ...
  document.querySelector(".coupen-input").addEventListener("input", (e) => {
    ...
    } else document.querySelector(".apply-coupen-btn").disabled = true;
  });
  ...
  if (order.coupon.code) {
    ...
    document
      .querySelector(".coupen-input-show-button")
      .classList.remove("d-none");
...
function applyCoupenCode(orderId) {
  return (e) => {
    ...
    fetch("http://nandhu.shop/order/apply_coupon?orderId=" + orderId, {
      method: "PATCH",
      headers: {
...
function addCouponAppliedInfo(coupon) {
  ...
  document.querySelector(".coupen-input").classList.add("d-none");
  document.querySelector(".coupon-discount-info").classList.remove("d-none");
  document.querySelector(".coupon-discount").innerHTML =
...
function removeCoupenCode(orderId) {
  return (e) => {
    fetch("http://nandhu.shop/order/remove_coupon?orderId=" + orderId, {
      method: "PATCH",
    })
      .then((response) => response.json())
      .then((data) => {
...
function createUserAddressTile(x, index) {
  const template = `
                        <input type="radio" name="address" class="mt-2" value=${
                          x._id
                        }>
                        <div class="address-content">
                            <p><span class="fw-bold">${x.fullName}</span> ${
    x.addressLine1 +
    " " +
    x.addressLine2 +
    " " +
  ...
  div
    .querySelector("input")
    .addEventListener("click", currentAddressUpdater(div));
  if (index == 0) {
...
function paymentMethodConfig() {
  ...
  fetch("http://nandhu.shop/wallet/get_balance")
    .then((response) => response.json())
    .then((data) => {
      if (data.isSuccess) {
...
function completeCheckout(addressId, paymentMethod) {
  fetch("http://nandhu.shop/order/complete_order/" + order._id, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
...
async function createOrder() {
  return await fetch("http://nandhu.shop/payment/create_order", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
function verifyPayment(response, addressId) {
  fetch("http://nandhu.shop/payment/verify", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then((response) => response.json())
    .then((data) => {
      if (data.isSuccess) completeCheckout(addressId, "Online-Payment");
      else
        Swal.fire({
          icon: "error",
          title: "Oops...",
