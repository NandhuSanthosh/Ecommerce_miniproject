...
function loader() {
  document
    .querySelector(".add_money_btn")
    .addEventListener("click", addMoneyHandler);
  document
    .querySelector(".send_money_btn")
    .addEventListener("click", sendMoneyHandler);
  document
    .querySelector(".transaction_history_btn")
    .addEventListener("click", transactionHistoryHandler);
  document
    .querySelector(".referal_btn")
    .addEventListener("click", referalHandler);
  ...
  // transaction history
  document
    .querySelector(".transaction-history .all_btn")
    .addEventListener("click", (e) => {
      document
        .querySelector(
          ".transaction-history-btn-list-container .btn-link.active"
        )
  ...
  document
    .querySelector(".transaction-history .send_btn")
    .addEventListener("click", (e) => {
      document
        .querySelector(
          ".transaction-history-btn-list-container .btn-link.active"
  ...
  document
    .querySelector(".transaction-history .purchase_btn")
    .addEventListener("click", (e) => {
      document
        .querySelector(
          ".transaction-history-btn-list-container .btn-link.active"
  ...
  document
    .querySelector(".transaction-history .refund_btn")
    .addEventListener("click", (e) => {
      document
        .querySelector(
          ".transaction-history-btn-list-container .btn-link.active"
  ...
  fetch( "/get_user_referals")
    ...
    .then((data) => {
      if (data.isSuccess) {
        ...
        document.querySelector(".invite-link-btn").dataset.value =
          data.referalLink;
      } else {
        Swal.fire({
          icon: "error",
...
function fetchAndDisplay(pno = 0, currentWindow, e) {
  fetch(
     "/wallet/get_tansaction_history?pno=" +
      pno +
      ...
      if (data.isSuccess) {
        ...
        if (data.totalCount) {
          data.data.transactions.forEach((x) => {
            document
              .querySelector(".transaction-history-list")
              .append(createTransactionTile(x));
          });
        } else {
          document.querySelector(".transaction-history-list").innerHTML = `
...
function configurePagination(count, currentPage) {
  ...
  nextLi.innerHTML = `<li class="page-item ${
...
function createTransactionTile({ transactions }) {
  ...
  if (transactionSpecificDoc.category == "addToWallet") {
    ...
    details =
      userTransactionDoc.type == "credit"
        ? transactionSpecificDoc.senderID[0].name
        : transactionSpecificDoc.receiverID[0].name;
  }
        ...
        <div class="d-flex justify-content-between">
            ...
            <div class="amount ${
              userTransactionDoc.type == "credit"
                ...
                <div>${userTransactionDoc.type == "credit" ? "+" : "-"} ₹${
    transactionSpecificDoc.amount
  }</div>
            </div>
    </div>`;
...
function findUser() {
  ...
  if (isValidCredential) {
    fetch( "/wallet/find_user?userCredential=" + credential)
      .then((response) => response.json())
      .then((data) => {
        if (data.isSuccess) {
          ...
          document.querySelector(".details .user_name").innerHTML =
            data.data.name;
          document.querySelector(".details .user_credential").innerHTML =
            credential;
        } else {
...
async function createOrder(amount) {
  return await fetch( "/wallet/create_payment_order", {
    method: "POST",
    headers: {
  ...
  fetch( "/wallet/verify_payment", {
    ...
    .then((data) => {
      if (data.isSuccess) {
        ...
        balance = data.data;
        wallet_balance.innerHTML = data.data.toLocaleString(undefined, {
